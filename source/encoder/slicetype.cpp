    ...
    if (!(param->rc.bStatRead && param->rc.cuTree && IS_REFERENCED(curFrame)))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (param->rc.aqMode == X265_AQ_AUTO_VARIANCE || param->rc.aqMode == X265_AQ_AUTO_VARIANCE_BIASED || param->rc.aqMode == X265_AQ_EDGE || param->rc.aqMode == X265_AQ_PERCEPTUAL)
                {
                ...
                for (int blockY = 0; blockY < maxRow; blockY += loopIncr)
                {
                    for (int blockX = 0; blockX < maxCol; blockX += loopIncr)
                    {
                        ...
                        else if (param->rc.aqMode == X265_AQ_PERCEPTUAL)
                        {
                            /* Suzaku AQ Mode 5: 感知引导自适应量化
                             * 基于 mode 3（auto-variance-biased）+ 渐变区域保护
                             *
                             * 核心改进：
                             * 1. 继承 mode 3 的暗部偏置保护
                             * 2. 检测低方差+微弱梯度的平涂渐变区域 → 额外降 QP（防色带）
                             * 3. 检测高方差纹理区域 → 允许稍高 QP（人眼不敏感）
                             */
                            qp_adj = curFrame->m_lowres.qpCuTreeOffset[blockXY];

                            /* 基础 AQ（继承 mode 3 的 auto-variance + bias） */
                            double base_adj = strength * (qp_adj - avg_adj) + bias_strength * (1.f - modeTwoConst / (qp_adj * qp_adj));

                            /* 渐变区域检测和保护 */
                            uint32_t energy = acEnergyCu(curFrame, blockX, blockY, param->internalCsp, param->rc.qgSize);
                            double bit_depth_corr = 1.f / (1 << (2 * (X265_DEPTH - 8)));
                            double norm_energy = energy * bit_depth_corr;

                            /* 低能量（平涂区域）且不是完全纯色（有微弱梯度=渐变） */
                            double variance_threshold = 50.0;   /* 低方差阈值 */
                            double gradient_threshold = 500.0;  /* 微弱梯度上限 */

                            if (norm_energy < variance_threshold && norm_energy > 1.0)
                            {
                                /* 平涂渐变区域：额外降 QP，防止色带 */
                                double gradient_factor = X265_MIN(norm_energy / variance_threshold, 1.0);
                                base_adj -= bias_strength * 0.5 * (1.0 - gradient_factor);
                            }
                            else if (norm_energy > gradient_threshold)
                            {
                                /* 高复杂度纹理区域：允许稍高 QP */
